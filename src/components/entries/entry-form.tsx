...
export function EntryForm({ entry, onSaved, onCancel }: EntryFormProps) {
  ...
  const litresFilled = form.watch('litresFilled')
    ...
    <form onSubmit={form.handleSubmit(onSubmit)} className='flex flex-col gap-4'>
      <FieldGroup>
          ...
          <Controller
            ...
            render={({ field }) => (
              <Popover>
                <PopoverTrigger
                  render={
                    <Button id='entry-date' type='button' variant='outline' className='justify-start font-normal' />
                  }>
                  <CalendarIcon />
        ...
        <Field data-invalid={!!form.formState.errors.fuelStation}>
          ...
          {!entry && quickStations.length > 0 && (
            <div className='flex flex-wrap gap-1.5 pt-0.5'>
              {quickStations.map((station) => (
                <button
                  key={station}
                  type='button'
                  onClick={() => form.setValue('fuelStation', station, { shouldValidate: true })}
                  className='rounded-full border border-border px-2.5 py-1 text-xs font-medium text-muted-foreground transition-colors hover:bg-accent hover:text-accent-foreground'>
                  {station}
                </button>
              ))}
            </div>
